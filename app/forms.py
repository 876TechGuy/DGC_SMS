...
class CreateRFQForm(FlaskForm):
    """Form for creating a new Request For Quotation."""
    title = StringField(
        'RFQ Title',
        validators=[DataRequired(), Length(min=5, max=255)],
        description='Brief title describing the procurement need',
    )
    description = TextAreaField(
        'Description',
        validators=[Optional()],
        description='Detailed description of goods/services required',
    )
    branch = SelectField(
        'Branch (Laboratory)',
        choices=[(b.value, b.value) for b in Branch],
        validators=[DataRequired()],
    )
    specification_reviewer = SelectField(
        'Specification Reviewer',
        coerce=int,
        validators=[DataRequired()],
        description='User responsible for verifying quotation compliance',
    )
    branch_head_approver = SelectField(
        'Branch Head for Final Approval',
        coerce=int,
        validators=[DataRequired()],
        description='Branch Head who must approve before supplier selection',
    )
    submit = SubmitField('Create RFQ')


class UploadQuotationForm(FlaskForm):
    """Form for uploading a quotation from a supplier."""
    supplier_name = StringField(
        'Supplier Name',
        validators=[DataRequired(), Length(min=3, max=255)],
    )
    supplier_contact = StringField(
    ...
    quotation_file = FileField(
        'Quotation Document',
        validators=[
            DataRequired(),
            FileAllowed(['pdf', 'doc', 'docx', 'xls', 'xlsx', 'txt'], 'Upload files must be PDF, Word, Excel, or TXT')
        ],
    )
    submit = SubmitField('Upload Quotation')


class SpecificationReviewForm(FlaskForm):
    """Form for specification reviewer to approve or reject a quotation."""
    action = RadioField(
        'Action',
        choices=[
            ('approved', 'Approve (Quotation complies with specifications)'),
            ('rejected', 'Reject (Quotation does not comply)'),
...
class BranchHeadApprovalForm(FlaskForm):
    """Form for branch head to approve or reject RFQ for final award."""
    action = RadioField(
        'Action',
        choices=[
            ('approved', 'Approve (RFQ approved for supplier selection/award)'),
            ('rejected', 'Reject (RFQ rejected - cannot proceed)'),
            ('returned', 'Return for Clarification'),
        ],
        validators=[DataRequired()],
    )
